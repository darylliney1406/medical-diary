...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("user_id", UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("name", sa.String(255), nullable=False),
        sa.Column("category", sa.Enum("food", "drink", name="cataloguecategory"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("entry_date", sa.Date, nullable=False, index=True),
        sa.Column("entry_time", sa.Time, nullable=False),
        sa.Column("meal_type", sa.Enum("breakfast", "lunch", "dinner", "snack", "drink", name="mealtype"), nullable=False),
        sa.Column("description", sa.Text, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("user_id", UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("summary_type", sa.Enum("daily", "weekly", name="summarytype"), nullable=False),
